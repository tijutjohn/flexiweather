...
//		private var mi_lastOneMoreY: Number;
		...
		private var _traceString2: String;
...
//			trace("new RouteCurveRenderer");
		...
		override public function started(x: Number, y: Number): void
		{
			...
			_traceString += "started("+x+","+y+") Reflection: " + reflectionID + " mi_routeType: " + mi_routeType +"\n";
			_traceString2 += "started("+x+","+y+") Reflection: " + reflectionID + " mi_routeType: " + mi_routeType +"\n";
...
//					m_graphics.beginFill(m_arrowFillStyle.color, m_arrowFillStyle.alpha);
		...
		{
//			trace("drawArrow ["+x+","+y+"] previous: ["+prevX+","+prevX+"]");
...
//			if (isNaN(mi_lastOneMoreX))
		...
		private var mb_drawArrow: Boolean;

		override public function lastPoint(x:Number, y:Number):void
		{
			mb_drawArrow = true;
			_traceString += "lastPoint("+x+","+y+") mi_routeType: " + mi_routeType + " _allPoints: " + _allPoints.length+"\n";
		}
		override public function finished(x: Number, y: Number): void
		{
			_logPoints = false;

//			trace("finished("+x+","+y+")" + routeType);
			if (mi_routeType == ROUTE_NORMAL)
			{
			} else if (mi_routeType == ROUTE_FILL) {
			}

			var tmpDrawArrow: Boolean = mb_drawArrow;

//			trace("Route finished before Arrow: " + routeType);
			_traceString += "finished("+x+","+y+") mb_drawArrow: "+ mb_drawArrow + " mi_routeType: " + mi_routeType + " _allPoints: " + _allPoints.length+"\n";
			_traceString2 += "finished("+x+","+y+") mb_drawArrow: "+ mb_drawArrow + " mi_routeType: " + mi_routeType + " _allPoints: " + _allPoints.length+"\n";
//			if (mi_routeType == 0)
//				trace("Route renderer: " + _traceString2);
...
//			trace("finished END ("+x+","+y+")" + routeType);
		}
	}
}