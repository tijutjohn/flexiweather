			...
			if (renderer)
			{
				...
				switch (data.type)
				{
					...
					case ToggleButtonBarItemData.TOGGLE:
						...
						_buttons[data.ident] = new ButtonDictionaryData(renderer, data);
						break;
					case ToggleButtonBarItemData.TOGGLE_GROUP:
...
//				renderer.addEventListener(MouseEvent.CLICK, item_clickHandler);
		...
		private function onExclusiveButtonChange(event: Event): void
		{
			var btn: IVisualElement = event.target as IVisualElement;
			var data: ToggleButtonBarItemData = getDataForRenderer(btn);
			var group: String = data.toggleGroup;
		...
		private function onExclusiveButtonClick(event: MouseEvent): void
		{
			...
			var clickedButtonType: String = buttonData.type;
			var toggleGroup: String = buttonData.toggleGroup;

			for each (var data: ButtonDictionaryData in _buttons)
			{
				var currButton: ToggleButton = data.btn as ToggleButton;
				var currButtonType: String = data.data.type;
				var currToggleGroup: String = buttonData.toggleGroup;

				if (currButton && clickedButton != currButton)
				{
					var bIsExclusive: Boolean = (clickedButtonType == ToggleButtonBarItemData.EXCLUSIVE && currButtonType == ToggleButtonBarItemData.EXCLUSIVE);
					var bIsToggleGroup: Boolean = (clickedButtonType == ToggleButtonBarItemData.TOGGLE_GROUP && currButtonType == ToggleButtonBarItemData.TOGGLE_GROUP && currToggleGroup == toggleGroup);
					if (bIsExclusive || bIsToggleGroup)
					{
